...
export const createRouteGateway = (requestService: IRequestService) => {
  return {
    ...
    getRouteById: async (
      routeId: string,
    ): IRoutesGateway.GetRouteById.ReturnType => {
      try {
        const { data } =
    ...
    updateRouteById: async (
      routeId: string,
      payload: Partial<IRoutes.Model>,
    ): IRoutesGateway.UpdateRouteById.ReturnType => {
      try {
        const { data } =
          await requestService.request<IRoutes.ModelWithRelation>(
            `/routes/${routeId}`,
    ...
    createRoute: async (
      payload: IRoutes.RawModel,
    ): IRoutesGateway.CreateRoute.ReturnType => {
      try {
        const { data } =
          await requestService.request<IRoutes.ModelWithRelation>(
            'routes',
    ...
    deleteRouteById: async (
      routeId: number,
    ): IRoutesGateway.DeleteRouteById.ReturnType => {
      try {
        await requestService.request<IRoutes.ModelWithRelation>(
          `/routes/${routeId}`,
          'delete',
